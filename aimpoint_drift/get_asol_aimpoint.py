...
def get_opt():
    ...
    parser.add_argument(
        "--start", help="Start time for processing (default=stop - 30 days)"
    )
    parser.add_argument("--stop", help="Processing stop date (default=NOW)")
    parser.add_argument(
        "--dt", default=1.0, help="Sample delta time (ksec, default=1.0)"
    )
...
def add_asol_to_h5(filename, asol):
    asol = asol.as_array()
    h5 = tables.openFile(
        filename, mode="a", filters=tables.Filters(complevel=5, complib="zlib")
    )
    try:
...
logger = pyyaks.logger.get_logger(
    name="get_asol_aimpoint", level=loglevel, format="%(asctime)s %(message)s"
)
